...
def data_noniid(dataset, num_users):
    ...
    mol_data.generate('/workspace/algorithm/cross_esol/data-smiles/esol_smiles.pkl', validation=0.1, test=0.1)
    data = mol_data.data
    ...
    for i in range(num_users):
        num_items_per_user = min(len(idx_shard), num_mols)  # Choose a minimum of available items and num_mols
        rand_set = set(np.random.choice(idx_shard, num_items_per_user, replace=False))
...
if __name__ == '__main__':
    ...
    data.generate('/workspace/algorithm/cross_esol/data-smiles/esol_smiles.pkl', validation=0.1, test=0.1)
    data.save('/workspace/algorithm/cross_esol/data-smiles/esol.dataset')