...
logging.basicConfig(filename='/workspace/algorithm/cross_esol/logs/train.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
#         return self.G.state_dict(), self.D.state_dict(), g_epoch_loss, d_epoch_loss
    ...
    def test(self):
        log_file_path = "/workspace/algorithm/cross_esol/logs/test.txt"

        def add_log(message, log_file=log_file_path):
            with open(log_file, "a") as file:  # 'a' 模式表示追加
                file.write(message + "\n")
        ...
        with torch.no_grad():
            ...
            recons_image.save("/workspace/algorithm/cross_esol/mols_img/recons_image.png", dpi=(1000, 1000))
            recons_image.show()
